...
class Admin {
	...
	private Map<Integer,String> vehicles;// a key value pair for mapping integers to vehicle types
	private Map<Integer,Double> rates; // a key value pair for mapping integers to rates of vehicles

	private int id = 1;//id to assign to customers
	
	private TruckFloor groundFloor;//truck floor object representing ground floor
	private CommonFloor[] commonFloor;//array of common floor objects representing floors other than ground floor
...
//method to allot spot index chosen by user based on his preference
	...
	public void start() {//starting of the parking lot by administrator 
		...
		while(true) {//if customer wants to leave or a new customer wants to park his vehicle
			System.out.println("Press 0 if the vehicle is already parked else press any other number for new entry, Or press 'X' to Logout");
			char x = scan.next().charAt(0);//asking for a valid entry to perform the desired operations
			if(x=='X'||x=='x')break;
	...
	public void customerIsPaying(Customer customer) {
		...
		customer.showTicket();
		String ticket=customer.getTicket();
		ParkingLot.P.writeToLog(ticket);
...
public class ParkingLot {
	...
	public static void main(String[] args) {
		...
		Scanner scan = new Scanner(System.in);
		boolean flag=false;
		if(!P.isLocked(LocalDateTime.now()))
		{
			...
			for(int i = 3; i >= 1; i--) 
			{
				...
				flag=P.checkPIN(pin);
				P.writeToLog(LocalDateTime.now(), flag, false);
				if(!flag) {
					if(i > 1)
						System.out.println("Please enter a valid pin. You have " + (i-1) + " tries remaining");
					else
					{
						System.out.println("You have been locked out. Contact support.");
						P.writeToLog(LocalDateTime.now(), flag, true);
					}
				}
				else 
					break;
